...
class ICN( Int32StringReceiver ):
  ...
  def handlePub(self, packet):
    eth = dpkt.ethernet.Ethernet( packet )
    info = pickle.loads(  data[len(eth):] )



  def handleSub(self, packet):
    eth = dpkt.ethernet.Ethernet( packet )
    info = pickle.loads(  data[len(eth):] )
  ...
  def handlePacketIn(self, rpc):
    ...
    if eth.type == MessageType.SUB: self.handleSub()
    if eth.type == MessageType.PUB: self.handleSub()
